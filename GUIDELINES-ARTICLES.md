...
Ce document consigne les règles de monétisation et d'intégration publicitaire obligatoires pour chaque nouvel article créé sur **sargasse.com**.

---

## 1. Principe Général : Zéro ID en Dur

La monétisation de ce site s'appuie sur le serveur autonome **Revive Adserver** (`https://ads.les4h.fr`, Éditeur ID `118`).
Tous les identifiants de zones Revive attribués à **sargasse.com** sont centralisés dans le fichier de configuration :
👉 **`src/config/ads.config.ts`**

> **Règle absolue** : Il est strictement interdit d'écrire des numéros de zone Revive (ex: `zoneId={709}`) directement dans les articles Markdown, les templates Astro ou les composants. Tout appel d'emplacement publicitaire doit obligatoirement se faire via son identifiant sémantique (`slot`).
...
| :--- | :--- | :---: | :--- |
| **`header`** | Leaderboard (728x90) | Zone 708 | Bannière haute injectée au-dessus ou sous le menu principal |
| **`inContent`** | Medium Rectangle (300x250) | Zone 709 | Encart contextuel inséré au milieu de l'article |
| **`largeRectangle`** | Large Rectangle (336x280) | Zone 707 | Encart de fin d'article inséré avant les articles recommandés |
| **`mobileSticky`** | Mobile Banner (320x100) | Zone 710 | Bandeau collant ancré en bas d'écran sur smartphone |
| **`halfPage`** | Half Page (300x600) | Zone 706 | Grand format pour colonne latérale (Sidebar desktop) |
| **`skyscraper`** | Skyscraper (160x600) | Zone 711 | Format vertical pour listing et dossiers thématiques |
