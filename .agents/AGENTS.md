# Directives & Contraintes d'Environnement — sargasse.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **sargasse.com** (sargasse) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `sargasse.com` |
| **URL Cloudflare Pages** | `https://sargasse.pages.dev` |
| **Dépôt GitHub** | `jc842/sargasse` (branche principale : `main`) |
...
| **Matomo Site ID** | `110` |
| **Matomo MTM Container** | `container_X7a6ujV3.js` (Container ID : `X7a6ujV3`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `118` |
| **Zones Publicitaires** | Header (`708`), In-Content (`709`), Mobile Sticky (`710`), Large Rectangle (`707`), Half Page (`706`), Skyscraper (`711`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_X7a6ujV3.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 709).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 707).
